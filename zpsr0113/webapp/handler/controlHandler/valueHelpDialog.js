sap.ui.define(
	...
	function (Filter, FilterOperator, Fragment) {
		...
		return {
			 ...
			 */
			onMultiValueHelpRequest: function (sProgramId, oControl, sTitle, oColumns, aFilters = []) {
				const oScreenModel = this._getScreenModel();
				...
				this._pMultipleConditionsDialog.then(
					function (oMultipleConditionsDialog) {
						...
						oMultipleConditionsDialog.setSupportRanges(false);
						oMultipleConditionsDialog.setSupportMultiselect(bIsMultiSelect);
						...
						oMultipleConditionsDialog.getTableAsync().then(
							function (oTable) {
								...
								if (oTable.bindRows) {
									...
									const aSortConfig = {
										Customer: [
											{ field: "SalesOrganization", desc: false },
										...
										SalesOffice: [
											{ field: "vkorg", desc: false },
											{ field: "vtweg", desc: false },
											{ field: "spart", desc: false },
											{ field: "vkbur", desc: false },
										],
										SalesGroup: [
											{ field: "vkbur", desc: false },
											{ field: "vkgrp", desc: false },
										],
									};
									const aFields = aSortConfig[sName];
									if (aFields) {
										aFields.forEach((sField) => {
											aSortItems.push(new sap.ui.model.Sorter(sField.field, sField.desc));
										});
									}
									...
									aTableProperties.forEach((sTableProperties, index) => {
										const sColumnKey = aColumnName[index];

										if (aColumnName.includes("Name2") && sTableProperties === "Name1") {
											oTable.addColumn(
												new sap.ui.table.Column({
													label: this.oBundle.getText("help" + sColumnKey),
													template: new sap.m.Text({
														text: {
															parts: [
																{ path: `${sModelName}>Name1` },
																{ path: `${sModelName}>Name2` },
															],
															formatter: function (name1, name2) {
