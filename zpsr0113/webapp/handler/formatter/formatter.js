sap.ui.define(["../helper/Validator", "sap/ui/core/format/NumberFormat"], function (Validator, NumberFormat) {
	...
	return {
		...
		getCurrentDate() {
			...
			const sYear = oCurrentTime.getFullYear();
			const sMonth = (oCurrentTime.getMonth() + 1).toString().padStart(2, "0");
			const sDay = oCurrentTime.getDate().toString().padStart(2, "0");
			return `${sYear}/${sMonth}/${sDay}`;
		},

		// Format date to yyyy/MM/dd
		FormatStringToDate: function (sDateInput) {
			if (!Validator.isRequired(sDateInput)) {
				return "";
			}
			sDateInput = sDateInput?.replace(/[^0-9]/g, "") || "";

			if (sDateInput) {
				// Case yyyyMMdd
				if (/^\d{8}$/.test(sDateInput)) {
					sDateInput = this.onFormatDisplayDate(sDateInput);
				}
