sap.ui.define(["sap/ui/base/Object", "sap/ui/core/format/NumberFormat"], function (BaseObject, NumberFormat) {
	"use strict";
	const Validator = BaseObject.extend("zpsr0019.libs.Validator", {
	 ...
	 * @returns
	 */
	Validator._controlValidations = function (oTarget, sStateKey, sStateTextKey, aCriteriaChecks) {
		const aMessages = [];
		let bExistFails = false;

		aCriteriaChecks.forEach((oCritera) => {
			if (Object.prototype.hasOwnProperty.call(oCritera, "sValue") && Validator._noInput(oCritera.sValue)) {
				return;
			}
			...
			if (oTarget instanceof sap.ui.core.Control) {
				...
				if (bResultCheck) {
					delete oTarget[sStateKey];
					delete oTarget[sStateTextKey];
				} else {
					oTarget[sStateKey] = "Error";
					oTarget[sStateTextKey] = oCritera.sStateMessage || oCritera.sMessage;
				}
			}
	...
	Validator.isDateString = function (sTarget) {
		...
		if (sTarget === void 0 || Validator._noInput(sTarget)) {
			// 無い場合は無視
			return retValue;
		}

		if (sTarget.includes(" - ")) {
        	return sTarget.split(" - ").every((sDate) => Validator.isDateString(sDate));
    	}
