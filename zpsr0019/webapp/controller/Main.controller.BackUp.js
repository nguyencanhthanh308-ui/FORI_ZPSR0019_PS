sap.ui.define(
	...
	function (
		...
		return Controller.extend("zpsr0019.controller.Main", {
			...
			// /**
			//  * handle custom URL
			//  * Handle count number on Bookmark
			//  */
			...
			// 		if (sName === "RequiredDate") {
			// 			oFilter["RequiredDateFrom"] &&
			// 				aParams.push(`RequiredDateFrom=${common.getFullDateFromNewDate(oFilter["RequiredDateFrom"])}`);
			// 			oFilter["RequiredDateTo"] &&
			// 				aParams.push(`RequiredDateTo=${common.getFullDateFromNewDate(oFilter["RequiredDateTo"])}`);
			// 		}
			// 		// Handle value for [組計状況]
			// 		if (sName === "AssemblyPlanningStatus" && oFilter[sName].length === 0) {
			// 			return;
			// 		}
			// 		if (oFilter[sName]) {
			...
			// 		}
			// 	});
			// 	return `#${sHash?.split("?")[0] || "zpsr0019"}?${aParams.join("&")}`;
			// },
			...
			// 		if (key === "RequiredDateFrom") {
			// 			oFilter.setProperty(`/${key}`, new Date(parsedValue));
			// 			continue;
			// 		}
			// 		if (key === "RequiredDateTo") {
			// 			oFilter.setProperty(`/${key}`, new Date(parsedValue));
			// 			continue;
			// 		}
			// 		if (key === "size-limit") {
         ...
         */
			...
			// 	common.handleExportFile(oTable, this.oBundle, "zpsr0019", oView);
			// },
			...
			_runRowValidations: async function (aRows, bIsImport = false) {
				...
				const aSelectCheckFields = bIsImport ? Constants.SelectCheckFieldsImport : Constants.SelectCheckFields;
			...
			// 	this.getView().setModel(new JSONModel(oRetrievedResult.results), "oDataTable");
			// },
			...
			onValueHelp: function (oEvent, sKey) {
				...
				switch (sKey) {
					case "purchase":
						sPreValue = oDataScreen?.PurchaseRequisitionType || "";
						sIdScreen = "valueHelpPurchase";
						sNameScreen = "zpsr0019.view.ValueHelpPurchase";
						break;
					default:
						break;
				}
					...
					function (oDialog) {
						...
						oDialog.attachConfirm(function (oEvent) {
							...
							if (sPreValue !== sSelectedValue) {
								switch (sKey) {
									case "purchase":
										oScreenModel.setProperty("/PurchaseRequisitionType", sSelectedValue);
										break;
									default:
										break;
								}
			...
			/**
			 * Handle Open or Hiden form reflection 
			 */
			...
			/**
			 * Checkbox True when on change Field 
			 */
			onChangeFieldForm: function (oEvent) {
				const oScreenModel = this._getScreenModel();
				const sID = oEvent.getSource().getId();
				//Validation date
				if (sID.includes("BULKBDTER")) {
					this._checkValidDateField(oEvent);
				}
				// Match by exact controlId, not sID.includes(key) — key is now an English
				// name (e.g. "Destination") that isn't contained in the control's legacy
				// SAP-style id (e.g. "BULKZ_SHUKKOSAKI").
				const oMatchedField = Constants.FieldsReflection.find((oField) => oField.controlId === sID);
				if (oMatchedField) {
					oScreenModel.setProperty(`/select${oMatchedField.key}`, true);
				}
			},
			...
			onPressReflection: function () {
				...
				const aFields = Constants.FieldsReflection.map((obj) => obj.key);
				...
				if (!_validateInputsDate(["BULKBDTER"])) {
					let aMessages = [];
				...
				aRowsChecked.forEach((oItem) => {
					...
					Constants.FieldsReflection.forEach(({ key, output }) => {
						if (!oScreenModel[`select${key}`]) return; // Skip if field not checked

						// [購買依頼タイプ] - Only reflect when 引落数量 = 0 (field is editable)
						if (key === "PurchaseRequisitionType" && parseFloat(oItem.OUTENMNG) !== 0) return;

						// [保管場所] - Sync key field alongside value field
						if (key === "StorageLocation") {
							oItem["OUTLGORT"] = oScreenModel[`Value${key}`];
						}

						// [購買依頼タイプ] - Sync value field alongside key field
						if (key === "PurchaseRequisitionType") {
							oItem["OUTKOUBAI_IRAI_TYPEvalue"] = oScreenModel[`Value${key}`];
						}
