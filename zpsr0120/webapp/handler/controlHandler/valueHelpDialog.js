	...
	function (Filter, FilterOperator, Fragment) {
		...
		return {
			...
			onMultiValueHelpRequest: function (
				...
				const bSearchHelpOnly = aSearchHelpOnly.includes(sName);
				...
				this._pMultipleConditionsDialog.then(
					function (oMultipleConditionsDialog) {
						...
						oMultipleConditionsDialog.setSupportRanges(bIsMultiSelect && !bSearchHelpOnly);
						...
						oMultipleConditionsDialog.getTableAsync().then(
							function (oTable) {
								...
								if (oTable.bindRows) {
									// Sort 得意先, 納入先 by Ascending
									const aSortItems = [];
									const aSortName = ["Customer", "ShipToParty"];
									if (aSortName.includes(sName)) {
										aSortItems.push(new sap.ui.model.Sorter("vkorg", false));
										aSortItems.push(new sap.ui.model.Sorter("kunnr", false));
									}
								...
								// if (["Customer", "ShipToParty"].includes(sName))
