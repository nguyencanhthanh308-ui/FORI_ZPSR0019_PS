sap.ui.define(
	["sap/ui/export/Spreadsheet", "sap/m/MessageBox", "sap/ui/export/library", "../formatter/formatter"],
	function (Spreadsheet, MessageBox, exportLibrary, Formatter) {
		"use strict";

		const EdmType = exportLibrary.EdmType;
		const oBundle = new sap.ui.model.resource.ResourceModel({
			bundleName: "zpsr0108.i18n.i18n",
		}).getResourceBundle();
		...
		return {
					...
					aNumberItems?.forEach((sNumberItem) => {
						if (oItem[sNumberItem]?.toString()?.trim()) {
							oItem[sNumberItem] = Formatter.displayMoneyFormatter(
								oItem[sNumberItem]?.replaceAll(",", "").toString() || "",
								"USD",
								false,
								true,
								0
			...
			createColumnConfig: function (oTable, oSpecialItems) {
				const aColumns = oTable.getColumns().filter((oCol) => oCol.getVisible());
