sap.ui.define(
	["../helper/Validator", "sap/ui/core/format/NumberFormat", "sap/ui/core/format/DateFormat"],
	function (Validator, NumberFormat) {
		"use strict";
		...
		return {
			 ...
			 * @param {Date|String} oDateInput
			 * @param {String} sFormat "yyyyMMdd" | "yyyyMM"
			 * @returns {String}
			 */
			dateInputFormatter: function (oDateInput, sFormat = "yyyyMMdd") {
				if (!oDateInput) {
					return "";
				}

				if (sFormat === "yyyyMM") {
					const sValue = oDateInput.toString().replace(/[/-]/g, "");
					return sValue.substring(0, 6);
				}

				if (typeof oDateInput === "string") {
					oDateInput = new Date(oDateInput);
				}

				const iYear = oDateInput.getFullYear();
				const sMonth = (oDateInput.getMonth() + 1).toString().padStart(2, "0");
				const sDay = oDateInput.getDate().toString().padStart(2, "0");
