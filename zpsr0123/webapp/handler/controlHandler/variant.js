	...
	function (JSONModel) {
		...
		return {
			...
			connectPersonalizationService: async function (oView, sNameVariantSet, sVariantId) {
				...
				oController.aColumnsDefault = this.getColumnSetting(oTable);
					...
					await this._oPersonalizationContainer
						...
						.done(
							async function (oContainer) {
								...
								await this._reArrangeVariantSet(oController);
								...
								oVariantManager.attachSelect(
									function (oEvent) {
										this.onSelect(oEvent, oView);
									}.bind(this)
								);

								oVariantManager.attachSave(
									function (oEvent) {
										this.onSaveAs(oEvent, oView);
									}.bind(this)
								);

								oVariantManager.attachManage(
									function (oEvent) {
										this.onManage(oEvent, oView);
									}.bind(this)
								);
			...
			_reArrangeVariantSet: async function (oController) {
				const aVariantKeys = this.oVariantSet.getVariantKeys();
				const aVariantData = [];
				aVariantKeys.forEach((oItem) => {
					const oVariantItem = this.oVariantSet.getVariant(oItem);
					aVariantData.push({
						name: oVariantItem.getVariantName(),
						value: oVariantItem.getItemValue("ColumnsVal"),
					});
					this.oVariantSet.delVariant(oVariantItem.getVariantKey());
				});
				if (aVariantData.length) {
					aVariantData.forEach((oItem) => {
						const oNewVariant = this.oVariantSet.addVariant(oItem.name);
						oNewVariant.setItemValue("ColumnsVal", oItem.value);
					});
				}
				await oController.oContainer
					.save()
					.done(() => {})
					.fail(() => {});
			},
			...
			onSaveAs: async function (oEvent, oView) {
				...
				// get columns data
				const oTable = oView.getController()._getTableControl();
			...
			handleBindVariantToTable: function (oView, sKey) {
				...
				if (sKey) {
					...
					aColumns.forEach(
						function (aColumn) {
							const aTableColumn = $.grep(oTable.getColumns(), function (element) {
				...
				// null means the standard variant is selected or the variant which is not available, then show all columns
					...
					const aColumnsDefault = oView.getController().aColumnsDefault;
					const iSortOrder = oView.getModel("screen").getProperty("/SortOrder"); // [ソート順]
					const aSortLabels = iSortOrder === 0 ? ["機台NO"] : ["開始予定", "優先NO", "機台NO"];
					...
					if (sId.includes("tblMain")) {
						aSortLabels.forEach((sLabel, index) => {
							const oColumnToSort = oTable.getColumns().find((col) => {
								return col.getLabel()?.getText?.() === sLabel;
							});

							if (oColumnToSort) {
								const bAdd = index > 0;
								oTable.sort(oColumnToSort, sap.ui.table.SortOrder.Ascending, bAdd);
							}
						});
					}
				}
			...
			onManage: async function (oEvent, oView) {
				...
				if (aParameters.def !== "*standard*") {
					...
					if (sDef.includes("SV")) {
						...
						oVariantManagement.setDefaultVariantKey(aParameters.def);
					} else {
						this.oVariantSet.setCurrentVariantKey(sDef);
			...
			handleSetVariantList: function (oView) {
				...
				for (const key in aVariantNameAndKey) {
			...
			getVariantManager: function (oView) {
				const oController = oView.getController();
				return oView.byId(oController._singleSheetVariantId ? oController._singleSheetVariantId : "Variants");
			},
